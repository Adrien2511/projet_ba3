...
#Ce fichier va permettre la transformation du modèle de Darknet en une version Keras

#Utilisation de argpase afin de faciliter l'utilisation des différents fichiers qui vont être utilisés

pars = argparse.ArgumentParser(description='Convertion de Darknet vers Keras.') #donne la description  de ce que va faire le fichier
...
#fonction qui permet d'enregistrer toutes les valeurs du modèle et la création du modèle avec Keras
def main2(args):
    config_p = os.path.expanduser(args.config_path)
    # args.config_path permet d'aller chercher l'argument du cfg de pars et os.path.expanduser() permet de lire le nom du fichier  qui sera enregisté dans config_p
    ...
    fichier_weights = open(weights_p, 'rb') #permet l'ouverture du fichier weights et la suite permet de les charger
    ...
    for section in cfg_pars.sections():                  #pour chaque bloc du fichier yolov3.cfg
        ...
        if section.startswith('convolutional'):          #rentre quand c'est un block de convolution
            filters = int(cfg_pars[section]['filters'])  # enregistre le filtre qui compte le nombre de noyaux
            ...
            prev_couche_shape = K.int_shape(prev_couche) # donne les dimensions de prev_couche
            ...
            # permet d'afficher  la convolution , si il y a du batch normalize , le type d'activation et le poids (qui correspond au noyau)
            ...
            act_fn = None
            if activation == 'leaky': # si l'activation est de type leaky
                pass
            elif activation != 'linear':               # ne prend pas les activations différentes de linear et leaky
            ...
            if batch_normalize: # si il y a du batch normalize
                conv_couche = (BatchNormalization(weights=bn_weight_list))(conv_couche) # application da Batch normalize
            prev_couche = conv_couche # enregistrement de la valeur

            if activation == 'linear':         # si c'est de type linear on ajoute juste la couche précédente
            ...
            size = int(cfg_pars[section]['size'])
            stride = int(cfg_pars[section]['stride'])
            all_couche.append(
                MaxPooling2D(
                    pool_size=(size, size),
                    strides=(stride, stride),
                    padding='same')(prev_couche))
            prev_couche = all_couche[-1]
            ...
            if len(couches) > 1:   #si le nombre d'éléments dans la section route est supérieur à 1
                # concaténation de la couche qui va permettre de l'enregistrer dans un seul vecteur
                print('Concatenating route layers:', couches)
                concatenate_couche = Concatenate()(couches) #concatene les couches ensemble
                all_couche.append(concatenate_couche) #ajout de la nouvelle couche
                prev_couche = concatenate_couche      # enregistre la valeur comme dernière couche
            else:                                        # si le route est composée d'un seul élément
                skip_couche = couches[0]                 # enregistre la valeur de la couche à l'indice de l'élément donné par route
                all_couche.append(skip_couche)           # ajout de la valeur au vecteur
                prev_couche = skip_couche                # enregistre la valeur

        elif section.startswith('shortcut'):          # si le bloc est de type shortcut
            # le shortcut permet d'améliorer le réseau en passant des couches , ce sont des connexions de raccourcis
            index = int(cfg_pars[section]['from'])    # enregistre le nombre couches qu'on passe
            ...
            prev_couche = all_couche[-1] # enregistre la dernière couche
            ...
            assert stride == 2, 'Only stride=2 supported.'   # si stride n'est pas de valeur 2 (avec stride=2 on va multiplier par 4 le nombre données)
            all_couche.append(UpSampling2D(stride)(prev_couche)) #application de la fonction Upsampling
            prev_couche = all_couche[-1] #on enregistre la dernière couche
            ...
            raise ValueError('Valeur de la section n est  pas prise : {}'.format(section)) # si c'est un bloc pas compris dans notre modèle

    #cette partie va créer le modèle et afficher les différents éléments du modèle
    if len(index_fin) == 0: index_fin.append(len(all_couche)-1) # si l'index de fin = 0 on ajoute la taille du vecteur
    model = Model(inputs=input_couche, outputs=[all_couche[i] for i in index_fin]) #création du modèle
    ...
    #permet de vérifier que tous les poids ont été utilisés
    remaining_weights = len(fichier_weights.read()) / 4
    fichier_weights.close() #fermeture du fichier
    print('Read {} of {} from Darknet weights.'.format(counter, counter +remaining_weights)) # affiche le nombre de poids utilisés par rapport au nombre de poids total
    if remaining_weights > 0:
        print('Attention: {} poids pas utilisé'.format(remaining_weights)) # affiche que tout les poids ne sont pas utilisé

    if args.plot_model:  #permet d'enregistrer le modèle sous forme d'image
...
def config_unique(config):
    ...
    with open(config) as fin: #permet d'ouvrir le fichier config
        for ligne in fin:  # pour chaque ligne
            if ligne.startswith('['):  # quand une ligne commence par "[" ce qui implique qu'on commence un bloc
                name = ligne.strip().strip('[]')  # on enlève les crochets autour du nom du bloc
